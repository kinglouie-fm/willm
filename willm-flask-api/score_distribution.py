...
# Function to gather all scores for violin plot
...
# Function to plot violin plots for each writing element
def plot_violin_scores():
    # Gather the data for violin plots
    violin_df = gather_all_scores()

    plt.rcParams.update({'font.size': 16})
    # Melt the DataFrame for easier plotting with seaborn
    violin_melted = violin_df.melt(var_name='Writing Element', value_name='Score')

    # Create the violin plot
    plt.figure(figsize=(10, 6))
    sns.violinplot(x='Writing Element', y='Score', data=violin_melted, inner='box', palette='Set2')
...
# Example usage to plot violin plots
plot_violin_scores()
