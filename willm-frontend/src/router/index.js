...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.matched.some(record => record.meta.requiresAuth)) {
    try {
      const profileResponse = await axios.get('https://willm.corinth.informatik.rwth-aachen.de:3101/user/profile', { withCredentials: true });
      const gamificationResponse = await axios.get('https://willm.corinth.informatik.rwth-aachen.de:3101/user/gamification', { withCredentials: true });
