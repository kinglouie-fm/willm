...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.matched.some(record => record.meta.requiresAuth)) {
    try {
      const profileResponse = await axios.get('http://34.46.136.120:3101/user/profile', { withCredentials: true });
      const gamificationResponse = await axios.get('http://34.46.136.120:3101/user/gamification', { withCredentials: true });
