...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    async register(username, password) {
      try {
        const response = await axios.post('https://academic-willm.de/user/register', { username, password, dataPrivacyConsent });
    ...
    async login(username, password) {
      ...
      let hideLoading2;
      try {
        const response = await axios.post('https://academic-willm.de/user/login', { username, password });
        if (response.status === 200 && response.data.message === 'Login successful') {
          ...
          if (response.data.postTestsCompleted) {
            ...
            const quizResponse = await axios.get('https://academic-willm.de/quiz/check-quiz');
    ...
    async logout() {
      try {
        await axios.post('https://academic-willm.de/user/logout');
