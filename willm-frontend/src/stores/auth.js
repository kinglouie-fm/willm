...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    async register(username, password) {
      try {
        const response = await axios.post('https://willm.corinth.informatik.rwth-aachen.de:3101/user/register', { username, password, dataPrivacyConsent });
    ...
    async login(username, password) {
      ...
      let hideLoading2;
      try {
        const response = await axios.post('https://willm.corinth.informatik.rwth-aachen.de:3101/user/login', { username, password });
        if (response.status === 200 && response.data.message === 'Login successful') {
          ...
          if (response.data.postTestsCompleted) {
            ...
            const quizResponse = await axios.get('https://willm.corinth.informatik.rwth-aachen.de:3101/quiz/check-quiz');
    ...
    async logout() {
      try {
        await axios.post('https://willm.corinth.informatik.rwth-aachen.de:3101/user/logout');
