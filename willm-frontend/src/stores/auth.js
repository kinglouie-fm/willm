...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    async register(username, password) {
      try {
        const response = await axios.post('http://34.71.205.236:3000/user/register', { username, password, dataPrivacyConsent });
    ...
    async logout() {
      try {
        await axios.post('http://34.71.205.236:3000/user/logout');
