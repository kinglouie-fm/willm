...
const handleCorrect = async () => {
  ...
  try {
    const correctionResponse = await axios.post('http://academic-willm.de/correct', {
      text: textToCorrect,
  ...
  try {
    ...
    await axios.post('http://academic-willm.de/question/generate');
  } catch (error) {
...
const generateReview = async () => {
  ...
  try {
    const response = await axios.post('http://academic-willm.de/review/generate', { reviewModel: authStore.getLLM('reviewModel') });
...
const getRecentReview = async () => {
  try {
    const response = await axios.get('http://academic-willm.de/review/recent');
...
const completePreTestProcess = async () => {
  try {
    if (preTestCount.value >= 1) {
      await axios.post('http://academic-willm.de/user/complete-pre-test');
...
const submitPreTest = async () => {
  ...
  try {
    await axios.post('http://academic-willm.de/user/pre-test', { text: preTestText.value, section: preTestSection.value });
    preTestCount.value++;
    preTestText.value = '';
...
const submitPostTest = async () => {
  ...
  try {
    const response = await axios.post('http://academic-willm.de/user/post-test', { text: postTestText.value, section: postTestSection.value });
...
const fetchPreTestSections = async () => {
  ...
  if (isAfter(currentDate, enableDate)) {
    try {
      const preTestResponse = await axios.get('http://academic-willm.de/user/pre-test-sections');
      preTestSections.value = preTestResponse.data.sections;

      const postTestResponse = await axios.get('http://academic-willm.de/user/post-test-sections');
