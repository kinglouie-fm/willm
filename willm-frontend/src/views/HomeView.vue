...
const handleCorrect = async () => {
  ...
  try {
    const correctionResponse = await axios.post('https://academic-willm.de/correct', {
      text: textToCorrect,
...
const generateReview = async () => {
  ...
  try {
    const response = await axios.post('https://academic-willm.de/review/generate', { reviewModel: authStore.getLLM('reviewModel') });
...
const getRecentReview = async () => {
  try {
    const response = await axios.get('https://academic-willm.de/review/recent');
...
const completePreTestProcess = async () => {
  try {
    if (preTestCount.value >= 1) {
      await axios.post('https://academic-willm.de/user/complete-pre-test');
...
const checkPreTestStatus = async () => {
  try {
    const response = await axios.get('https://academic-willm.de/user/pre-test-status');
...
const submitPreTest = async () => {
  ...
  try {
    await axios.post('https://academic-willm.de/user/pre-test', { text: preTestText.value, section: preTestSection.value });
    preTestCount.value++;
    preTestText.value = '';
...
const submitPostTest = async () => {
  ...
  try {
    const response = await axios.post('https://academic-willm.de/user/post-test', { text: postTestText.value, section: postTestSection.value });
...
const fetchPreTestSections = async () => {
  ...
  if (isAfter(currentDate, enableDate)) {
    try {
      ...
      const postTestResponse = await axios.get('https://academic-willm.de/user/post-test-sections');
