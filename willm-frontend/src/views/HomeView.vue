...
const handleCorrect = async () => {
  ...
  try {
    ...
    await axios.post('https://willm.corinth.informatik.rwth-aachen.de/question/generate');
  } catch (error) {
...
const completePreTestProcess = async () => {
  try {
    if (preTestCount.value >= 1) {
      await axios.post('https://willm.corinth.informatik.rwth-aachen.de/user/complete-pre-test');
...
const submitPreTest = async () => {
  ...
  try {
    await axios.post('https://willm.corinth.informatik.rwth-aachen.de/user/pre-test', { text: preTestText.value, section: preTestSection.value });
    preTestCount.value++;
    preTestText.value = '';
...
const submitPostTest = async () => {
  ...
  try {
    const response = await axios.post('https://willm.corinth.informatik.rwth-aachen.de/user/post-test', { text: postTestText.value, section: postTestSection.value });
...
const fetchPreTestSections = async () => {
  ...
  if (isAfter(currentDate, enableDate)) {
    try {
      const preTestResponse = await axios.get('https://willm.corinth.informatik.rwth-aachen.de/user/pre-test-sections');
      preTestSections.value = preTestResponse.data.sections;

      const postTestResponse = await axios.get('https://willm.corinth.informatik.rwth-aachen.de/user/post-test-sections');
