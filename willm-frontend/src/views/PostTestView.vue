...
const loadTest = async () => {
    try {
        loadingMessage = message.info("Loading Post-Test...", 0); // Persistent loading message
        const response = await axios.get(
            `http://willm.corinth.informatik.rwth-aachen.de/test/${userId}/post-test`
        );
...
const submitTest = async () => {
    try {
        loadingMessage = message.info("Submitting Post-Test...", 0); // Persistent loading message
        await axios.post(
            `http://willm.corinth.informatik.rwth-aachen.de/test/${userId}/${test.value._id}/post-test/complete`,
            answers.value
        );
        if (loadingMessage) loadingMessage(); // Dismiss loading message
    ...
    <div class="container-fluid h-100 mt-4">
        <div class="row h-100">
            <div class="col-12">
                ...
                <div v-if="!test">
                    <!-- Loading message is handled via message.info -->
                </div>
                <div v-else>
