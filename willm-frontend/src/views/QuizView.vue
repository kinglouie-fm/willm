...
const fetchQuiz = async () => {
    try {
        const response = await axios.get('http://34.71.205.236:3000/quiz/get-todays-quiz');
...
const submitAnswer = async (selectedAnswer = null, option) => {
    try {
        ...
        const response = await axios.post('http://34.71.205.236:3000/quiz/submit-answer', {
...
const requestExplanation = async () => {
    try {
        const response = await axios.post('http://34.71.205.236:3000/quiz/explain-answer', {
            quizId: quiz.value.quiz_id,
            questionId: currentQuestion.value.question_id,
...
const completeQuiz = async () => {
    try {
        const response = await axios.post('http://34.71.205.236:3000/quiz/complete', {
            quizId: quiz.value.quiz_id,
        });
