...
const fetchQuiz = async () => {
    try {
        const response = await axios.get('http://34.46.136.120:3101/quiz/get-todays-quiz');
...
const submitAnswer = async (selectedAnswer = null, option) => {
    ...
    try {
        ...
        const response = await axios.post('http://34.46.136.120:3101/quiz/submit-answer', {
...
const requestExplanation = async () => {
    try {
        const response = await axios.post('http://34.46.136.120:3101/quiz/explain-answer', {
            quizId: quiz.value.quiz_id,
            questionId: currentQuestion.value.question_id,
...
const completeQuiz = async () => {
    try {
        const response = await axios.post('http://34.46.136.120:3101/quiz/complete', {
            quizId: quiz.value.quiz_id,
        });
